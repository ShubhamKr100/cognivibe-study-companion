...
export const AccessibilityToolbar: React.FC = () => {
  const { 
    ...
    isADHDSupport, toggleADHDSupport,
  ...
  return (
    <div className="fixed bottom-24 right-6 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 z-50 overflow-hidden animate-fade-in-up max-h-[85vh] overflow-y-auto">
      ...
      <div className="p-5 space-y-6">
        ...
        <div className="pb-6 border-b border-slate-100">
           ...
           <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                <span className="text-slate-700 font-medium text-sm">Calm Voice (ADHD)</span>
                <button 
                  onClick={toggleADHDSupport}
                  className={`w-10 h-5 rounded-full relative transition-colors ${isADHDSupport ? 'bg-indigo-500' : 'bg-slate-300'}`}
                >
                  <div className={`w-3 h-3 bg-white rounded-full absolute top-1 transition-all ${isADHDSupport ? 'left-6' : 'left-1'}`} />
                </button>
              </div>
           </div>
        </div>

        {/* Dyslexia Focus Tools Section */}
        <div className="pb-6 border-b border-slate-100">
           <h4 className="text-xs text-teal-600 font-bold uppercase mb-3 flex items-center">
             <span className="mr-1">👁️</span> Dyslexia Focus Tools
           </h4>
           <div className="space-y-3">
             <div className="flex justify-between items-center">
                <span className="text-slate-700 font-medium text-sm">Dyslexia Font</span>
                <button 
                  onClick={toggleDyslexicFont}
                  className={`w-10 h-5 rounded-full relative transition-colors ${isDyslexicFont ? 'bg-teal-500' : 'bg-slate-300'}`}
                >
                  <div className={`w-3 h-3 bg-white rounded-full absolute top-1 transition-all ${isDyslexicFont ? 'left-6' : 'left-1'}`} />
                </button>
              </div>
        ...
        <div>
          <h4 className="text-xs text-slate-500 font-bold uppercase mb-3 flex items-center">
             <span className="mr-1">🎨</span> Visual Comfort
           </h4>
